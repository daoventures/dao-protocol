...
//
// As above example, it has 0.000001 USDT deltas for withdrawal.
/*-----------------------------------------------------------------------------------------------------------*/
...
describe("Harvest-Farmer USDT", () => {
  ...
  it("should deploy contract correctly for Vault and Strategy contract", async () => {
    ...
    expect(await vaultContract.vaultName()).to.equal(ethers.utils.formatBytes32String("DAOVault Medium-Risk USDT"))
    ...
    expect(await strategyContract.strategyName()).to.equal(ethers.utils.formatBytes32String("Harvest-Farmer USDT"))
  ...
  describe("User functions", () => {
    ...
    it("should deduct correct profile sharing fee when vesting in strategy contract", async () => {
      ...
      const profileSharingFee = decimals(100).mul(1).div(10)
      const profit = decimals(100).sub(profileSharingFee)
      const networkFee = decimals(500).mul(1).div(100) // 500 USDT not invested to strategy yet. fee is 1%
  ...
  describe("Admin functions", () => {
    it("should able to transfer contract ownership to other address by contract owner only in Vault and Strategy factory contracts", async () => {
      ...
      await expect(vaultFactoryContract.connect(clientSigner).createVault(ethers.utils.formatBytes32String("DAOVault Medium-Risk USDT"),
        tokenAddress, strategyContract.address)).to.be.revertedWith("Ownable: caller is not the owner")
      await expect(strategyFactoryContract.connect(clientSigner).createStrategy(ethers.utils.formatBytes32String("Harvest-Farmer USDT"),
      ...
      await expect(vaultFactoryContract.createVault(ethers.utils.formatBytes32String("DAOVault Medium-Risk USDT"),
        tokenAddress, strategyContract.address)).to.be.revertedWith("Ownable: caller is not the owner")
      await expect(strategyFactoryContract.createStrategy(ethers.utils.formatBytes32String("Harvest-Farmer USDT"),
        tokenAddress, hfVaultAddress, hfStakeAddress, FARMAddress, uniswapRouterAddress, WETHAddress)).to.be.revertedWith("Ownable: caller is not the owner")
    })
