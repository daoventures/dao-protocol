...
const unlockedAddress = addresses.ADDRESSES.unlockedUser
const unlockedAddress2 = addresses.ADDRESSES.unlockedUser2
...
// 
    it("Should yield correctly", async() => {//TODO - check
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), 2)
        await vault.connect(adminSigner).invest()
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("100", 18), 2)
        await vault.connect(adminSigner).invest()
...
// 
    ...
    it("Should switch pools correctly", async() => {
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("1000", 18), 2)
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("100000", 18), 2)
        let shares = await vault.balanceOf(unlockedUser2.address)
        let balanceBefore = await DAI.balanceOf(unlockedUser2.address)
        await vault.connect(adminSigner).invest()
// 
        await vault.connect(adminSigner).switchMode(1)
        await vault.connect(adminSigner).switchMode(0)
// 
        await vault.connect(unlockedUser2).withdraw(shares, 2)
        let balanceAfter = await DAI.balanceOf(unlockedUser2.address)
// 
    ...
    it("Should work - normal flow", async() => {
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), 2)
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("10000", 18), 2)
        let balanceBefore = await DAI.balanceOf(unlockedUser.address)
        let balanceBefore2 = await DAI.balanceOf(unlockedUser2.address)
        let shares = await vault.balanceOf(unlockedUser.address)
        let sharesUser2 = await vault.balanceOf(unlockedUser2.address)
        await vault.connect(adminSigner).invest()
        await vault.connect(unlockedUser).withdraw(shares, 2)
        await vault.connect(unlockedUser2).withdraw(sharesUser2, 2)
        let balanceAfter = await DAI.balanceOf(unlockedUser.address)
        // console.log('withdrawn ', (balanceAfter.sub(balanceBefore).toString()))
        let valueInPool = await vault.getAllPoolInUSD()
        // console.log('valueInPool', valueInPool.toString())
// 
        let balanceAfter2 = await DAI.balanceOf(unlockedUser2.address)
    ...
    it("Should revert other functions on emergency", async() => {
        ...
        await vault.connect(adminSigner).emergencyWithdraw()
        await expect(vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), 2)).to.be.revertedWith("Cannot deposit during emergency")
...
// 
    it("should work correctly - migrateFunds", async() => {
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), 2)
        let beforeBalance = await DAI.balanceOf(unlockedUser.address)
        await vault.connect(adminSigner).invest()
// 
