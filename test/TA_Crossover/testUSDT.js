...
const unlockedAddress2 = "0x3b3d80e345DFfa2342E165EF8bB07679D89934FC"//addresses.ADDRESSES.unlockedUser2
...
// 
    it("Should yield correctly", async() => {//TODO - check
        ...
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("100", 6), 0)
        await vault.connect(adminSigner).invest()
...
// 
    ...
    it("Should switch pools correctly", async() => {
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("1000", 6), 0)
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("100000", 6), 0)
...
// 
    ...
    it("Should work - normal flow", async() => {
        const {vault, strategy, USDC, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer} = await setup()
        await vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 6), 0)
        await vault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("10000", 6), 0)
        ...
        await vault.connect(adminSigner).invest()
        await vault.connect(unlockedUser).withdraw(shares, 0)
        await vault.connect(unlockedUser2).withdraw(sharesUser2, 0)
...
// 
    ...
    it("Should revert other functions on emergency", async() => {
        ...
        await vault.connect(adminSigner).emergencyWithdraw()
        await expect(vault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 6), 0)).to.be.revertedWith("Cannot deposit during emergency")
