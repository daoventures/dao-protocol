...
describe("cfDAI", () => {
    ...
    const setup = async () => {
        ...
        const daoStrategyFactoryContract = await ethers.getContract("DAOStrategyFactory")
        ...
        const daoVaultFactoryContract = await ethers.getContract("DAOVaultFactory")
        const dvlDAIAddress = (await daoVaultFactoryContract.getDAOVaults())[0]
        const dvlDAIContract = await ethers.getContractAt("DAOVault", dvlDAIAddress)
    ...
    it("should deploy contract correctly", async () => {
        ...
        expect(await dvlDAIContract.name()).to.equal("DAOVault Low DAI")
        ...
        expect(await cfDAIContract.strategyName()).to.equal(ethers.utils.formatBytes32String("Compound-Farmer DAI"))
        expect(await dvlDAIContract.name()).to.equal("DAOVault Low DAI")
        expect(await dvlDAIContract.symbol()).to.equal("dvlDAI")
        expect(await dvlDAIContract.decimals()).to.equal(18)
        ...
        expect(await dvlDAIContract.networkFeeTier2(0)).to.equal(decimals(50000).add(1))
        expect(await dvlDAIContract.networkFeeTier2(1)).to.equal(decimals(100000))
        expect(await dvlDAIContract.customNetworkFeeTier()).to.equal(ethers.utils.parseUnits("1", 24))
        expect(await cfDAIContract.DENOMINATOR()).to.equal("10000")
        expect(await dvlDAIContract.networkFeePercentage(0)).to.equal("100")
        expect(await dvlDAIContract.networkFeePercentage(1)).to.equal("75")
        expect(await dvlDAIContract.networkFeePercentage(2)).to.equal("50")
        expect(await dvlDAIContract.customNetworkFeePercentage()).to.equal("25")
    ...
    describe("User functions", () => {
        it("should able to deposit correctly", async () => {
            ...
            await tokenContract.connect(clientSigner).approve(dvlDAIContract.address, depositAmount)
            ...
            await sampleContract.approve(dvlDAIContract.address, depositAmount)
            await expect(sampleContract.deposit(depositAmount)).to.be.revertedWith("Only EOA")
            // Deposit token into contracts
            const tx = await dvlDAIContract.connect(clientSigner).deposit(depositAmount)
            const receipt = await tx.wait()
            console.log(receipt.gasUsed.toString())
            // Check if amount of deposit is correct
            expect(await dvlDAIContract.getCurrentBalance(clientSigner.address)).to.equal(decimals("100").mul(99).div(100)) // deposit fee 1%
        ...
        it("should deduct correct network fee based on tier in CompoundFarmerDAI contract", async () => {
            ...
            await tokenContract.approve(dvlDAIContract.address, depositTier1.add(depositTier2).add(depositTier3).add(customDepositTier))
            // Tier 1 deposit
            ...
            expect(await dvlDAIContract.getCurrentBalance(deployerSigner.address)).to.equal(depositTier1.sub(fee))
            ...
            expect(await dvlDAIContract.getCurrentBalance(deployerSigner.address)).to.equal(depositBalance.add(depositTier2.sub(fee)))
            ...
            expect(await dvlDAIContract.getCurrentBalance(deployerSigner.address)).to.equal(depositBalance.add(depositTier3.sub(fee)))
            ...
            expect(await dvlDAIContract.getCurrentBalance(deployerSigner.address)).to.equal(depositBalance.add(customDepositTier.sub(fee)))
            expect(await tokenContract.balanceOf(treasuryWalletAddress)).to.equal(treasuryBalance.add(fee.mul(1).div(2)))
            expect(await tokenContract.balanceOf(communityWalletAddress)).to.equal(communityBalance.add(fee.mul(1).div(2)))
        })
        ...
        it("should be able to withdraw correctly", async () => {
            ...
            const balance = await dvlDAIContract.getCurrentBalance(clientSigner.address)
            // const cTokenContract = new ethers.Contract(cTokenAddress, ICERC20_ABI, deployerSigner)
            const underlyingBalance = await cTokenContract.balanceOfUnderlying(cfDAIContract.address)
            const tx = await dvlDAIContract.connect(clientSigner).withdraw(balance)
            const receipt = await tx.wait()
            console.log(receipt.gasUsed.toString())
        ...
        it("should be able to withdraw in several times correctly", async() => {
            ...
            const depositBalanceBoforeWithdraw = await dvlDAIContract.getCurrentBalance(clientSigner.address)
            ...
            const totalSupplyBeforeWithdraw = await dvlDAIContract.totalSupply()
            ...
            expect(await dvlDAIContract.getCurrentBalance(clientSigner.address)).to.equal(depositBalanceBoforeWithdraw.sub(withdrawAmount))
            ...
            expect(await dvlDAIContract.getCurrentBalance(clientSigner.address)).to.equal(0)
            expect(await cfDAIContract.pool()).to.equal(0)
        ...
        it("should deduct correct profile sharing fee when withdraw in CompoundFarmerDAI contract", async () => {
            ...
            await tokenContract.connect(clientSigner).approve(dvlDAIContract.address, decimals("1000"))
        ...
        it("should be able to mix and match deposit and withdraw correctly", async () => {
            ...
            await tokenContract.approve(dvlDAIContract.address, decimals("10000"))
            await tokenContract.connect(clientSigner).approve(dvlDAIContract.address, decimals("10000"))
            ...
            expect(await cfDAIContract.pool()).to.equal(0)
            expect(await dvlDAIContract.getCurrentBalance(deployerSigner.address)).to.equal(0)
            expect(await dvlDAIContract.getCurrentBalance(clientSigner.address)).to.equal(0)
            expect(await dvlDAIContract.totalSupply()).to.equal(0)
        ...
        it("should be able to refund correctly when contract is in vesting state", async () => {
            ...
            const depositAmount = decimals("1000")
            await tokenContract.approve(dvlDAIContract.address, depositAmount)
            await dvlDAIContract.deposit(depositAmount)
            await tokenContract.transfer(clientSigner.address, depositAmount)
            await tokenContract.connect(clientSigner).approve(dvlDAIContract.address, depositAmount)
            await dvlDAIContract.connect(clientSigner).deposit(depositAmount)
            const depositBalance = await dvlDAIContract.getCurrentBalance(deployerSigner.address)
            ...
            const refundBalance = await dvlDAIContract.getCurrentBalance(deployerSigner.address)
    ...
    describe("Admin functions", () => {
        it("should able to transfer contract ownership to other address by contract owner only in both contracts", async () => {
            ...
            await expect(dvlDAIContract.connect(clientSigner).setNetworkFeeTier2([decimals(100), decimals(200)])).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setNetworkFeePercentage([3000, 3000, 3000])).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setCustomNetworkFeeTier(ethers.utils.parseUnits("1", 12))).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setCustomNetworkFeePercentage(3000)).to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(cfDAIContract.connect(clientSigner).revertVesting()).to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(dvlDAIContract.connect(clientSigner).setNetworkFeeTier2([decimals(100), decimals(200)])).not.to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setNetworkFeePercentage([3000, 3000, 3000])).not.to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setCustomNetworkFeeTier(ethers.utils.parseUnits("1", 12))).not.to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(clientSigner).setCustomNetworkFeePercentage(3000)).not.to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(cfDAIContract.connect(clientSigner).revertVesting()).not.to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(dvlDAIContract.connect(deployerSigner).setNetworkFeeTier2([decimals(100), decimals(200)])).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(deployerSigner).setNetworkFeePercentage([3000, 3000, 3000])).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(deployerSigner).setCustomNetworkFeeTier(ethers.utils.parseUnits("1", 12))).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(dvlDAIContract.connect(deployerSigner).setCustomNetworkFeePercentage(3000)).to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(cfDAIContract.connect(deployerSigner).revertVesting()).to.be.revertedWith("Ownable: caller is not the owner")
        ...
        it("should able to set new treasury wallet correctly in CompoundFarmerDAI contract", async () => {
            ...
            await expect(dvlDAIContract.setTreasuryWallet(clientSigner.address))
                .to.emit(dvlDAIContract, "SetTreasuryWallet")
                .withArgs(treasuryWalletAddress, clientSigner.address)
            ...
            expect(await dvlDAIContract.treasuryWallet()).to.equal(clientSigner.address)
            // Check if new treasury wallet receive fees
            await tokenContract.approve(dvlDAIContract.address, decimals(1000))
        ...
        it("should able to set new community wallet correctly in CompoundFarmerDAI contract", async () => {
            ...
            await expect(dvlDAIContract.setCommunityWallet(clientSigner.address))
                .to.emit(dvlDAIContract, "SetCommunityWallet")
                .withArgs(communityWalletAddress, clientSigner.address)
            ...
            expect(await dvlDAIContract.communityWallet()).to.equal(clientSigner.address)
            // Check if new treasury wallet receive fees
            await tokenContract.approve(dvlDAIContract.address, decimals(1000))
            await dvlDAIContract.deposit(decimals(200))
            // Deposit amount within network fee tier 1 hence fee = 0.5%
        ...
        it("should able to set new network fee tier correctly in CompoundFarmerDAI contract", async () => {
            const { dvlDAIContract } = await setup()
            // Check if function parameter meet the requirements
            await expect(dvlDAIContract.setNetworkFeeTier2([0, decimals(10000)]))
                .to.be.revertedWith("Minimun amount cannot be 0")
            await expect(dvlDAIContract.setNetworkFeeTier2([decimals(10000), decimals(10000)]))
            ...
            await expect(dvlDAIContract.setNetworkFeeTier2([decimals(60000).add(1), decimals(600000)]))
                .to.emit(dvlDAIContract, "SetNetworkFeeTier2")
            ...
            expect(await dvlDAIContract.networkFeeTier2(0)).to.equal(decimals(60000).add(1))
            expect(await dvlDAIContract.networkFeeTier2(1)).to.equal(decimals(600000))
        })

        it("should able to set new custom network fee tier correctly in CompoundFarmerDAI contract", async () => {
            const { dvlDAIContract } = await setup()
            // Check if function parameter meet the requirements
            await expect(dvlDAIContract.setCustomNetworkFeeTier(ethers.utils.parseUnits("1", 22)))
            ...
            await expect(dvlDAIContract.setCustomNetworkFeeTier(ethers.utils.parseUnits("2", 24)))
                .to.emit(dvlDAIContract, "SetCustomNetworkFeeTier")
            ...
            expect(await dvlDAIContract.customNetworkFeeTier()).to.equal(ethers.utils.parseUnits("2", 24))
        })

        it("should able to set new network fee percentage correctly in CompoundFarmerDAI contract", async () => {
            const { dvlDAIContract } = await setup()
            // Check if function parameter meet the requirements
            await expect(dvlDAIContract.setNetworkFeePercentage([3000, 0, 0]))
                .to.be.revertedWith("Network fee percentage cannot be more than 30%")
            await expect(dvlDAIContract.setNetworkFeePercentage([0, 3000, 0]))
                .to.be.revertedWith("Network fee percentage cannot be more than 30%")
            await expect(dvlDAIContract.setNetworkFeePercentage([0, 0, 3000]))
                .to.be.revertedWith("Network fee percentage cannot be more than 30%")
            // Set network fee percentage and check if event for setNetworkFeePercentage is logged
            await expect(dvlDAIContract.setNetworkFeePercentage([200, 100, 50]))
                .to.emit(dvlDAIContract, "SetNetworkFeePercentage")
                .withArgs([100, 75, 50], [200, 100, 50])
            // Check if network fee percentage is set correctly
            expect(await dvlDAIContract.networkFeePercentage(0)).to.equal(200)
            expect(await dvlDAIContract.networkFeePercentage(1)).to.equal(100)
            expect(await dvlDAIContract.networkFeePercentage(2)).to.equal(50)
        })
        ...
        it("should able to set new custom network fee percentage correctly in CompoundFarmerDAI contract", async () => {
            const { dvlDAIContract } = await setup()
            // Check if function parameter meet the requirements
            await expect(dvlDAIContract.setCustomNetworkFeePercentage(60))
            ...
            await expect(dvlDAIContract.setCustomNetworkFeePercentage(10))
                .to.emit(dvlDAIContract, "SetCustomNetworkFeePercentage")
                .withArgs(25, 10)
            // Check if network fee percentage is set correctly
            expect(await dvlDAIContract.customNetworkFeePercentage()).to.equal(10)
        })
        ...
        it("should able to set pending strategy, migrate funds and set new strategy correctly in DAOVaultLowDAI contract", async () => {
            ...
            await tokenContract.approve(dvlDAIContract.address, decimals(100000))
        ...
        it("should set contract in vesting state correctly in CompoundFarmerDAI contract", async () => {
            ...
            await dvlDAIContract.deposit(decimals(500))
            const depositAmount = await dvlDAIContract.getCurrentBalance(deployerSigner.address)
            ...
            const deployerBalanceAfterVesting = await dvlDAIContract.getCurrentBalance(deployerSigner.address)
        ...
        it("should revert contract vesting state and lend into Compound again correctly", async () => {
            ...
            await tokenContract.connect(clientSigner).approve(dvlDAIContract.address, decimals("2000"))
            await dvlDAIContract.connect(clientSigner).deposit(decimals("1000"))
            expect(await tokenContract.balanceOf(cfDAIContract.address)).to.equal(0)
            ...
            let clientBalance = await dvlDAIContract.getCurrentBalance(clientSigner.address)
