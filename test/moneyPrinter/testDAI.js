...
describe("Admin functions", async() => {
    ...
    it('Should work correctly with admin', async() => {
        const {moneyPrinterVault, moneyPrinterStrategy, admin, deployer, unlockedUser, DAI} = await setup()
        await moneyPrinterVault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), DAI.address)
...
describe("EmergencyWithdraw", async() => {
    ...
    it('Should work correctly', async () => {
        const {moneyPrinterVault, moneyPrinterStrategy, admin, DAI, unlockedUser} = await setup()
        await moneyPrinterVault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), DAI.address)
        await moneyPrinterVault.connect(admin).emergencyWithdraw(DAI.address)
        const balanceBefore = await DAI.balanceOf(unlockedUser.address)
        await moneyPrinterVault.connect(unlockedUser).withdraw(moneyPrinterVault.balanceOf(unlockedUser.address), DAI.address)
        const balanceAfter = await DAI.balanceOf(unlockedUser.address)

        expect(balanceAfter.gt(balanceBefore)) 
    }) 

    it('Should not allow other functions during emergency', async() => {
        const {moneyPrinterVault, moneyPrinterStrategy, admin, DAI, unlockedUser, deployer} = await setup()
        await moneyPrinterVault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), DAI.address)
        await moneyPrinterVault.connect(admin).emergencyWithdraw(DAI.address)
        await expect( moneyPrinterVault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), DAI.address)).to.be.revertedWith("Cannot deposit during emergency")
    ...
    it('Should remove emergency on reInvest', async() => {
        const {moneyPrinterVault, moneyPrinterStrategy, admin, DAI, unlockedUser, unlockedUser2, deployer} = await setup()
        await moneyPrinterVault.connect(unlockedUser).deposit(ethers.utils.parseUnits("100", 18), DAI.address)
        await moneyPrinterVault.connect(unlockedUser2).deposit(ethers.utils.parseUnits("50", 18), DAI.address)
        await increaseTime(3600000)
        await moneyPrinterVault.connect(admin).yield()
        await moneyPrinterVault.connect(admin).emergencyWithdraw(DAI.address)
...
describe("Normal flow", async () => {
    ...
    it('Should deposit, yield, withdraw correctly', async() => {
        ...
        await increaseTime(3600000)
        await moneyPrinterVault.connect(admin).yield()
        let user1balanceBefore = await DAI.balanceOf(unlockedUser.address)
        let user2balanceBefore = await DAI.balanceOf(unlockedUser2.address)
        await moneyPrinterVault.connect(unlockedUser).withdraw(moneyPrinterVault.balanceOf(unlockedUser.address), DAI.address)
        await moneyPrinterVault.connect(unlockedUser2).withdraw(moneyPrinterVault.balanceOf(unlockedUser2.address), DAI.address)
        let user1balanceAfter = await DAI.balanceOf(unlockedUser.address)
        let user2balanceAfter = await DAI.balanceOf(unlockedUser2.address)
    ...
    it("Should update poolValue correctly", async () => {
        ...
        await moneyPrinterVault.connect(unlockedUser).withdraw(await moneyPrinterVault.balanceOf(unlockedUser.address), DAI.address)
        await moneyPrinterVault.connect(unlockedUser2).withdraw(await moneyPrinterVault.balanceOf(unlockedUser2.address), DAI.address)
    ...
    it("Should transfer fee correctly on deposit", async () => {
        ...
        let amountDeposited = ethers.utils.parseUnits("100", 18)
        let onePercentOfDeposited = amountDeposited.div(100)

        await moneyPrinterVault.connect(unlockedUser).deposit(amountDeposited, DAI.address)
