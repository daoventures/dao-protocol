...
contract TAvault is ERC20("DAO Vault TA", "daoTA"), Ownable, BaseRelayRecipient { 
    ...
    event SetBiconomy(address indexed _biconomy);
    event SetPercTokenKeepInVault(uint256[] _percentages);
    ...
    function withdraw(uint256 _shares, uint256 _tokenIndex) external {
        ...
        if (_withdrawAmtInUSD > _depositAmt) {
            ...
            _fee = _token.decimals == 6 ? _fee.div(1e12): _fee;
            uint _feeFourtyPerc = _fee.mul(2).div(5);
            //transfer fee
            _token.token.safeTransfer(treasuryWallet, _feeFourtyPerc);
            _token.token.safeTransfer(communityWallet, _feeFourtyPerc);
            _token.token.safeTransfer(strategist, _fee.sub(_feeFourtyPerc).sub(_feeFourtyPerc));
        }
    ...
    function getAllPoolInETH(uint256 _price) public view returns (uint256) {
        ...
        (uint _valueInEth, )= strategy.getValueInPool();
        return _valueInEth.add(_vaultPoolInETH);
    }
