...
contract CompoundFarmerUSDT is ERC20, Ownable {
  ...
  uint256[] public depositFee = [100, 50, 25]; // Represent [Tier 1, Tier 2, Tier 3], initial value represent [1%, 0.5%, 0.25%]
  uint256 public profileSharingFee = 1000; // 10%
  uint256 public constant treasuryAndCommunityFee = 5000; // 50% split to treasury and community on profile sharing fee
  ...
  event SetProfileSharingFee(uint256 indexed oldProfileSharingFee, uint256 indexed newProfileSharingFee);
  ...
  function setDepositFee(uint256[] calldata _depositFee) external onlyOwner {
    require(
      _depositFee[0] < 4000 &&
      _depositFee[1] < 4000 &&
      _depositFee[2] < 4000, "Deposit fee percentage cannot be more than 40%"
    );
  ...
  function deposit(uint256 _amount) external {
    ...
    } else if (_amount >= depositFeeTier2[0] && _amount <= depositFeeTier2[1]) {
    // Tier 2
  ...
  function withdraw(uint256 _amount) external {
    ...
    if (compToken.balanceOf(address(this)) > 0) {
      ...
      if (_amountsOut[2] > 0) {
        uint256 _amountOutMin = _amountsOut[2].mul(_amountOutMinPerc).div(100);
        uniswapRouter.swapExactTokensForTokens(_amountIn, _amountOutMin, _path, address(this), block.timestamp);
      }
    }
    ...
    if (_r > _amount) {
      ...
      token.safeTransfer(treasuryWallet, _fee.mul(treasuryAndCommunityFee).div(DENOMINATOR));
      token.safeTransfer(communityWallet, _fee.mul(treasuryAndCommunityFee).div(DENOMINATOR));
    } else {
